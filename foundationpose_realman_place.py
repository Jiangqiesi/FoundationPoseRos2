...
class FoundationPoseMoveIt2ControllerRealMan(Node):
        ...
        try:
            ...
            for i, point in enumerate(jt.points):
                ...
                ret = self.rm_controller.movej(
                    positions_deg, v=20, r=radius,
                    connect=connect_flag, block=block_flag
                )
                if ret != 0:
    ...
    def move_to_home(self) -> bool:
        ...
        ret = self.rm_controller.movej(home_joints_deg, v=20, r=0, connect=0, block=1)
        return ret == 0
        ...
        while rclpy.ok():
            try:
                ...
                if user_input.lower() in ['q', 'quit']:
                    ...
                    print(f"跟踪的物体: {list(self.latest_poses.keys())}")
                    continue
                    ...
                    if not self.latest_poses:
                        ...
                        for obj_id in sorted(self.latest_poses.keys()):
                            ...
                            print(f"  朝向: ({orient.x:.4f}, {orient.y:.4f}, {orient.z:.4f}, {orient.w:.4f})")
                    continue
