...
if __name__ == "__main__":
    ...
    if len(grasps_inferred) > 0:
        ...
        grasp_conf_inferred_rot = grasp_conf_inferred.copy()
        grasps_inferred = grasps_inferred.cpu().numpy()
        grasps_inferred_raw = grasps_inferred.copy()

        # Rotate grasps by -45 degrees around the y-axis
        rot_mat = tra.rotation_matrix(- np.pi / 4.0, [0, 1, 0])
        grasps_inferred = np.array([rot_mat @ g for g in grasps_inferred])
        # Rotate grasps by 90 degrees around the x-axis
        rot_mat = tra.rotation_matrix(np.pi / 2.0, [1, 0, 0])
        grasps_inferred = np.array([rot_mat @ g for g in grasps_inferred])

        scores_inferred = get_color_from_score(grasp_conf_inferred, use_255_scale=True)
        scores_inferred_rot = get_color_from_score(np.array([conf - 0.5 for conf in grasp_conf_inferred]), use_255_scale=True)
        print(
        ...
        if not args.no_visualization:
            print("Visualizing inferred grasps...")
            for j, (grasp_raw, grasp_rotated) in enumerate(
                zip(grasps_inferred_raw, grasps_inferred)
            ):
                # Original grasp poses from inference
                visualize_grasp(
                    vis,
                    f"grasps_objectpc_original/{j:03d}/grasp",
                    grasp_raw,
                    color=scores_inferred[j],
                    gripper_name=gripper_name,
                    linewidth=0.6,
                )
                # Grasp poses after the rotation adjustment
                visualize_grasp(
                    vis,
                    f"grasps_objectpc_rotated/{j:03d}/grasp",
                    grasp_rotated,
                    color=scores_inferred_rot[j],
                    gripper_name=gripper_name,
                    linewidth=1.6,
                )
        ...
        if args.output_file != "":
            ...
            save_to_isaac_grasp_format(
                grasps_inferred_raw, grasp_conf_inferred, args.output_file.replace(".yml", "_raw.yml")
            )
        else:
