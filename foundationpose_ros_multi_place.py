...
class PoseEstimationNode(Node):
    ...
    def __init__(self, new_file_paths, camera_config, base_frame, output_frame, min_depth, max_depth, use_tf, scales):
        ...
        # 【修改-替换】 支持多模型不同scale
        for i, (mesh, s) in enumerate(zip(self.meshes, scales)):
            mesh.apply_scale(s)
            
            # --- 新增调试代码 Start ---
            self.get_logger().info(f"Checking Mesh {i}: {self.mesh_files[i]}")
            # 1. 检查尺寸：如果是5cm的方块，extents应该是 [0.05, 0.05, 0.05]
            self.get_logger().info(f"  -> Size (meters): {mesh.extents}") 
            # 2. 检查纹理是否加载
            if hasattr(mesh.visual, 'kind') and mesh.visual.kind == 'texture':
                self.get_logger().info(f"  -> Texture: OK (Texture visual detected)")
            elif hasattr(mesh.visual, 'kind') and mesh.visual.kind == 'face':
                 self.get_logger().warn(f"  -> Texture: WARNING! No texture map found (Color per face only).")
            else:
                 self.get_logger().warn(f"  -> Texture: CRITICAL WARNING! Mesh might be loading as plain geometry without texture!")
        # --- 新增调试代码 End -
